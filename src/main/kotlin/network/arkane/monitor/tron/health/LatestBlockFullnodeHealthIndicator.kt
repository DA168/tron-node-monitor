...
class LatestBlockFullnodeHealthIndicator(val grpcClient: GrpcClient) : AbstractReactiveHealthIndicator() {
    ...
    private val latestBlock: Optional<GrpcAPI.BlockExtention>
        get() = try {
            Optional.ofNullable(grpcClient.blockingStubFull.getNowBlock2(GrpcAPI.EmptyMessage.getDefaultInstance()))
        } catch (ex: Exception) {
            ex.printStackTrace()
            Optional.empty()
        }
            ...
            if (block.isPresent) {
                ...
                return if (blockTime.plus(10, ChronoUnit.MINUTES).isBefore(LocalDateTime.now(ZoneId.systemDefault()))) {
                    Mono.just(builder.status(Status.DOWN)
                            .withDetail("tron.fullnode", "last block is older than 30 minutes")
                            .build())
                } else {
                    block.map {
                ...
                return Mono.just(
                        builder.down()
                                .withDetail("tron.fullnode.down", "Unable to fetch status for ethereum node").build())
            }
        } catch (ex: Exception) {
            return Mono.just(
                    builder.down()
                            .withDetail("tron.fullnode.exception", ex.message).build())
        }
    }
}